...
class PNetScraper(BaseScraper):
    def scrape(self) -> list[dict]:
        ...
        for page in range(1, MAX_PAGES + 1):
            ...
            listings = soup.find_all("div", class_="job-item")
    ...
    def _parse_listing(self, listing) -> dict | None:
        try:
            title_tag = listing.find("h2", class_="job-title")
            company_tag = listing.find("span", class_="company-name")
            location_tag = listing.find("span", class_="location")
            salary_tag = listing.find("span", class_="salary")
            url_tag = listing.find("a", href=True)
            description_tag = listing.find("div", class_="job-description")
            ...
            url = "https://www.pnet.co.za" + url_tag["href"] if url_tag else None
